...
const static double CLOSE_DISTANCE = .25;	//Two points must be within this to be considered closePoints. Make sure to update vector reserve() calls when changing this
...
const static string DISTANCE_FILE = "I:\\DistanceSkimsDatabaseAM.csv";
...
static int TripModes[] = { 0, 1, 1, 1, 1, 1, 1, 1 };//1 represents that array index is shareable. Right now, indexes modes 1-7 (and not 0) are shareable
													
static int RandomFailChance = 20;					//%chance a trip will randomly not be shareable. This should be an integer from 0-100 
...
static bool close[NUM_LOCATIONS][NUM_LOCATIONS];
